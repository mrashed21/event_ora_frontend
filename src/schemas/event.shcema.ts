...
export const create_event_schema = base_event_schema.superRefine(
  (data, ctx) => {
    if (data.registration_fee !== undefined && data.registration_fee < 0) {
      ctx.addIssue({
...
export const update_event_schema = base_event_schema.partial();

export type EventFormData = z.infer<typeof create_event_schema>;
export type UpdateEventFormData = z.infer<typeof update_event_schema>;
